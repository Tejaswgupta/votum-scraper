...
const caseTypeValue = await selectOptionByText(caseTypeSelect, formData.caseType, true); // Assuming partial match might be needed
...
const resultsData = await page.evaluate(() => {
  ...
  tableRows.forEach(row => {
      ...
      if (row.querySelector('td[colspan="3"]')) return;
        ...
        const viewLinkOnClick = row.children[3]?.querySelector('a')?.getAttribute('onclick');
